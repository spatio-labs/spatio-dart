//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:spatio_sdk/api.dart';
import 'package:test/test.dart';

// tests for SendEmailRequest
void main() {
  // final instance = SendEmailRequest();

  group('test SendEmailRequest', () {
    // String accountId
    test('to test the property `accountId`', () async {
      // TODO
    });

    // List<String> to (default value: const [])
    test('to test the property `to`', () async {
      // TODO
    });

    // List<String> cc (default value: const [])
    test('to test the property `cc`', () async {
      // TODO
    });

    // List<String> bcc (default value: const [])
    test('to test the property `bcc`', () async {
      // TODO
    });

    // String subject
    test('to test the property `subject`', () async {
      // TODO
    });

    // String body
    test('to test the property `body`', () async {
      // TODO
    });

    // bool html
    test('to test the property `html`', () async {
      // TODO
    });

    // List<AttachmentInput> attachments (default value: const [])
    test('to test the property `attachments`', () async {
      // TODO
    });

    // String inReplyTo
    test('to test the property `inReplyTo`', () async {
      // TODO
    });

    // List<String> references (default value: const [])
    test('to test the property `references`', () async {
      // TODO
    });


  });

}
